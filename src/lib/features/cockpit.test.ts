describe('HUD_COCKPIT_V1_ENABLED', () => {
  const original = process.env.NEXT_PUBLIC_HUD_COCKPIT_V1;

  afterEach(() => {
    if (original === undefined) {
      delete process.env.NEXT_PUBLIC_HUD_COCKPIT_V1;
    } else {
      process.env.NEXT_PUBLIC_HUD_COCKPIT_V1 = original;
    }
    jest.resetModules();
  });
