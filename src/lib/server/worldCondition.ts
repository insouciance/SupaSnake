...
/**
 * The one modifier a Serpent week's condition-set resolves to.
 *
 ...
 * economic modifiers compose in the payout fold — a balance decision, and
 * WP-2.10b's business — a set of any size resolves to its FIRST recognised
 * entry. Deterministic, and never a silent stack.
 */
export function serpentWeekCondition(
  week: Pick<SerpentWeekRow, 'modifiers'> | null | undefined
): AnomalyId | null {
  for (const modifier of week?.modifiers ?? []) {
    if (isAnomalyId(modifier.id)) return modifier.id;
  }
  return null;
}
 ...
 *
 * Re-derived rather than read out of `signal_days.modifier`, which is the
 * discipline the rest of `signal.ts` keeps (`toDayRow` marries the stored id
 * to the calendar's derivation; `settleSignalObjectiveRun` re-derives the
...
export function signalDayCondition(
  ...
  return day === null ? null : describeSignalDay(day).condition.id;
}
...
export async function resolveSessionWorldCondition(
  ...
  playerId: string,
  sessionId: string
): Promise<AnomalyId | null> {
  if (isAnomalyId(session.anomaly_id)) return session.anomaly_id;
