...
describe('exact-SHA workflow proof', () => {
  ...
  it('accepts only a completed successful push run for the exact SHA', () => {
    expect(classifyExactPushRun({
      workflow_runs: [
        ...
        { id: 2, head_sha: sha, event: 'workflow_dispatch', status: 'completed', conclusion: 'success' },
        { id: 3, head_sha: sha, event: 'push', status: 'completed', conclusion: 'success' },
      ],
  ...
  it('fails closed for missing, pending, failed, or newer non-green runs', () => {
    ...
    expect(classifyExactPushRun({
      workflow_runs: [{ id: 4, head_sha: sha, event: 'push', status: 'in_progress', conclusion: null }],
    }, sha)).toMatchObject({ ok: false, reason: 'status_in_progress' });
    expect(classifyExactPushRun({
      workflow_runs: [{ id: 5, head_sha: sha, event: 'push', status: 'completed', conclusion: 'failure' }],
    }, sha)).toMatchObject({ ok: false, reason: 'conclusion_failure' });
    expect(classifyExactPushRun({
      workflow_runs: [
        { id: 6, run_attempt: 1, head_sha: sha, event: 'push', status: 'completed', conclusion: 'success' },
        { id: 7, run_attempt: 2, head_sha: sha, event: 'push', status: 'completed', conclusion: 'cancelled' },
      ],
