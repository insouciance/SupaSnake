...
a row is explicitly marked **GAP**. Line numbers are against `origin/main` `2fe33ca`.
...
- **target** — a target whose `GenomeV2TargetState.kind` equals the Gene's ID
  ...
  failure). Written at `genomeV2.ts:2886-2892` from the `target_resolved` event
...
|---|---|---|---|---|---|---|---|
| 1 | `gold_trail` | yield · AURUM | A Gilded target resolves, or its premium window expires | target + journal | event `genomeV2.ts:722`, `:743` (`target_window_expired`); lifecycle `:2886-2892` | collected in-window = `completed`; window expiry or miss = `burnt`/`expired` | — |
| 2 | `compound_interest` | banking · AURUM | A deliberate Loom DECLINE mints a Bond | journal + ledger | event `:645`; write `:2509-2511` (`state.bonds + 1`) | the Bond exists either way; crash simply pays nothing | — |
| 3 | `loan_shark` | banking · AURUM/UMBRA | A portal CONTINUE opens the six-food Escrow contract | journal + ledger | event `:667`; write `:2368` (`state.loan`), `:2219` (`loanEscrowDeposited`) | released (`:2228`) = success; BANK/crash before completion = failure | CONTINUE locked (< 1 validated bank) |
| 4 | `live_wire` | execution · VOLT | A Live Wire route test resolves | target | `:722`; lifecycle `:2886-2892`; kind `:439` | within `moveBudget` = `completed`; over budget = `burnt` | — |
| 5 | `circuit_run` | execution · VOLT/FLUX | A linked Circuit route resolves | target | `:722` (`circuitLegsCompleted`, `:734`); lifecycle `:2886-2892` | both legs = `completed`; broken route = `burnt` | — |
| 6 | `time_dilation` | body · VOLT/FERAL | **GAP — none exists** | — | growth only, `:2327-2331`; no journal event, no named ledger field | n/a | CYBER (excluded from the roster, `genes.ts:577-579`) |
| 7 | `overgrowth` | body · FERAL | **GAP — none attributable** | — | growth `:2324-2326`; Yield folds into `ledger.continuousDelta` `:2186`, `:3174`, which is shared with other continuous mechanics | n/a | — |
| 8 | `coilkeeper` | terrain · FERAL/FLUX | A coil seals territory | journal + ledger | event `:747` (`coil_sealed`); write `:2936` (`permanentTerrain`, `source: 'coilkeeper_seal'`) | the seal itself is the lesson; the empowered target that follows is the payoff | — |
| 9 | `wall_rush` | terrain · FLUX/VOLT | A charged wall impact redirects along its previewed tangent | journal | event `:758` (`wall_redirected`); write `:2990` (charge spent) | armed route resolved = success; missed = failure, charge still spent | No charge available (`state.wallRushCharges < 1`, `:2986`) |
| 10 | `phase_gate` | terrain · FLUX | A gate shortcut is taken | journal + ledger | event `:752` (`phase_gate_used`); write `:2973` (`permanentTerrain`, `source: 'phase_gate_scar'`) | taking the gate is the lesson; the Scar is its cost | — |
| 11 | `mirror_wager` | banking · UMBRA | A portal CONTINUE freezes a visible Stake | journal + ledger | event `:667` with `activateMirror: true` (`:670`); write `:2384` (`state.mirrorLeg`), `:2292` (`ledger.mirrorStake`) | BANK doubles it; crash loses only the Stake — both resolve | CONTINUE locked (< 1 validated bank) |
| 12 | `phoenix` | survival · UMBRA/FERAL | The second life is consumed | journal | event `:797` (`phoenix_triggered`); write `:3263` (Ash occupies the socket) | there is no failure mode: firing is the lesson | **`externalSecondLife !== null`** — an equipped outside revive (`iron_scales`) excludes Phoenix from the offer filter entirely (`:3952`) |
| 13 | `loom_anchor` | genome · AURUM/UMBRA | A pinned candidate is delivered into the next offer | journal | pin on `offer_declined.pinGeneId` (`:648`); delivery on `offer_opened.pinnedGeneId` (`:642`), honoured at `:2466-2473` | delivery = success; the run ending before the next offer = unresolved, not failed | Fewer than two offers remain reachable in the run |
| 14 | `heartwood` | body · FERAL · PRIMAL signature | Deliberate body geometry claims territory | journal + ledger | event `:762` with `source: 'heartwood'` (`:766`); write `:3015` (`state.territories`) | the claim is the lesson | Non-PRIMAL run |
| 15 | `zenith_protocol` | execution · VOLT · CYBER signature | An overclock window opens | journal | event `:769` with `source: 'zenith_protocol'` (`:771`); write `:3042` (`state.overclock`); closes at `:774` | mistimed overclock still teaches; both resolve | Non-CYBER run |
...
fourth food (`genomeV2.ts:2327-2331`, physics table `:4106-4112`). The reducer emits
...
Its growth is written at `:2324-2326` and its Yield folds into
`ledger.continuousDelta` (`:2186`, `:3174`) — a field the comment explicitly
...
into a digest and discarded (`genomeV2.ts:1606-1618`), and resolved targets compact
the same way above 96 (`:1620-1641`). A settlement-time scan for "did event X
...
`wallRushCharges` is restored by a portal CONTINUE (`:2613`), `state.overclock`
returns to null when the window ends (`:774`), `crownWave` closes, `anchor.pinnedGeneId`
clears on delivery (`:2525-2527`), and `ledger.mirrorStake` is zeroed when Phoenix
consumes it (`:3218`). None of them can be read at settlement as "this happened."
...
|---|---|---|
| `externalSecondLife !== null` (equipped `iron_scales` or other outside revive) | `phoenix` | `genomeV2.ts:3952` — already excluded from `legal` by the shipped filter |
