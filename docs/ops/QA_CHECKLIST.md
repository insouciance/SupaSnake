...
| Hosted Supabase | `supasnake`, `eu-central-1`; migrations 001–036 deployed; local migration 037 (FTUE v2) requires rollout approval |
| Payments | Stripe sandbox/test mode only |
| Support/legal contact | `support@supasnake.com` |
| Active development branch | `feat/genome` |

Production stays on the deployment above until a later promotion is explicitly
announced. A local follow-up to the HUD and resume work is under validation and
is not part of the production target yet.
...
- Hosted migration history is aligned through 036. A linked `db push
  --dry-run` selects only `037_ftue_v2_player_flow.sql`; nothing was applied.
...
- The focused HUD journey uses deterministic authenticated player/collection/
  session responses so it does not depend on migration 037 rollout state. A
  hosted FTUE-v2/bootstrap E2E pass is still required after migration approval.
...
- [ ] A disposable local Supabase reset applies migrations 001–036 from zero;
...
- [ ] Linked migration dry-run/list shows local and hosted histories aligned;
      migrations 029 and 030 remain forward-safe and idempotent where promised.
