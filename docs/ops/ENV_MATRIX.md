...
| Application | `NEXT_PUBLIC_APP_URL`, `MIN_AGE_REQUIREMENT`, manifest flags including `NEXT_PUBLIC_GENOME_V2`, `SUPASNAKE_PUBLIC_SURFACE_HASH` | `https://supasnake.com`, age 14; `config/production-public-surface.json` remains the single source for the 22-flag production contract and its deterministic hash. Current production proves 22/22 flags enabled. Genome v2 is exact `true`; omission or any non-`true` value keeps new starts on v1 while the dual-version runtime still completes existing v2 sessions |
...
- Linked production project: `gmpwyzqafoyowndbvlma` (`supasnake`,
  `eu-central-1`).
- Production has migrations 001–065. Production workflow `30784902800` applied
  migration 065 after an exact linked dry-run and two aggregate-only zero-v2
  preflights. Canonical health at release `2f9a9a61c57ecea04c8ea38804ce80c83b406c05`
  proves Genome schema/catalog/Ascendance version 2, eight Splices, rules version
  2, neutral 2/3/4 Strain thresholds, and the existing cohesive capabilities.
