...
| Application | `NEXT_PUBLIC_APP_URL`, `MIN_AGE_REQUIREMENT`, manifest flags including `NEXT_PUBLIC_GENOME_V2`, `NEXT_PUBLIC_PLAYER_EVOLUTION_V1` and `NEXT_PUBLIC_SNAKE_COSMETICS`, `SUPASNAKE_PUBLIC_SURFACE_HASH` | `https://supasnake.com`, age 14; `config/production-public-surface.json` remains the single source for the production contract and its deterministic hash. The **checked-in** contract is now **24 flags** at `e60cd71ee0ca67a5be81d165b26d0bf8eab337319276862367a9f2b89d158017`, having gained `NEXT_PUBLIC_SNAKE_COSMETICS` for the LF-B home cosmetics menu. The **deployed** artifact still proves 23/23 and `ac678998f5c58d0a1cab711e759271f426d2fa5b09a503bf20094406ffd8e2be` until the LF-B release lands — that release is the cutover, exactly as the 2026-08-05 flag-on release was the 22 → 23 cutover for `NEXT_PUBLIC_PLAYER_EVOLUTION_V1` (now live, and an ordinary manifest entry). Neither flag needs a Vercel dashboard entry: the deploy workflow's "Load exact production public-surface contract" step writes every manifest flag **and** a freshly computed `SUPASNAKE_PUBLIC_SURFACE_HASH` into the job environment *before* `verify:production-env` runs, and passes the same manifest-derived values to the deployment as `--build-env`/`--env` pairs. Do **not** pin the hash in the dashboard: the pulled `.vercel/.env.production.local` is overlaid on top of the job environment, so a pinned literal overwrites the freshly computed value and stops the release on the next contract change — including this one. Genome v2 requires exact `true`; omission/non-`true` keeps new starts on v1 while the dual-version runtime still completes existing v2 sessions |
