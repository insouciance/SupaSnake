...
| Application | `NEXT_PUBLIC_APP_URL`, `MIN_AGE_REQUIREMENT`, manifest flags including `NEXT_PUBLIC_GENOME_V2` and `NEXT_PUBLIC_PLAYER_EVOLUTION_V1`, `SUPASNAKE_PUBLIC_SURFACE_HASH` | `https://supasnake.com`, age 14; `config/production-public-surface.json` remains the single source for the **23-flag** production contract and deterministic hash `ac678998f5c58d0a1cab711e759271f426d2fa5b09a503bf20094406ffd8e2be`. The deployed artifact still proves the previous 22/22 and `8bf7f5634d0e36982326920668c1f5a8e79df5f9cdf402c66925899509e0fd99` until the flag-on release lands — that release is the cutover, and `NEXT_PUBLIC_PLAYER_EVOLUTION_V1=true` **must exist in Vercel Production before the deploy workflow runs** or `verify:production-env` stops it. Genome v2 requires exact `true`; omission/non-`true` keeps new starts on v1 while the dual-version runtime still completes existing v2 sessions |
